...
out_dir = '/home/okml2/gedi_agbd_output/'

#read in sites to be evaluated
id_list = []

with open('./proj_to_eval.txt', 'r') as f:
    for line in f:
        id_list.append(int(line.strip()))
print(id_list)

for id in id_list:
    print('starting for VCS_' + str(id))
    shapefile = '/maps/4C/projects/VCS_' + str(id) + '/GIS/aoi.shp'
    shp_file = gpd.read_file(shapefile)
    project = 'VCS_' + str(id)

    # Making dir for saving output
    project_dir = out_dir + '/' + project
    if not os.path.exists(project_dir):
        os.makedirs(project_dir)

    geometry = shp_file['geometry'].to_crs(crs = 3857).buffer(30000, cap_style = 1, join_style = 1).to_crs(crs = 4326) # Getting geometry of shape file with a 30 km buffer
    ...
    high_quality.to_csv(os.path.join(project_dir + '/' + project + '_30km_quality_shots.csv'))
    ...
    #print(agbd_val)
    agbd_val.to_csv(os.path.join(project_dir + '/' + project + '_agbd.csv'))
    
    print('VCS_' + str(id) +' completed')

print('all selected projects done') 
# shapefile = '/maps/4C/projects/VCS_1396/GIS/aoi.shp'
# #shp_file = gpd.GeoDataFrame.from_file(shapefile)
# shp_file = gpd.read_file(shapefile)
# # shp_file.geometry = convert_3D_2D(shp_file.geometry)
# project = shapefile.split("/")[4]

# #making dir for output
# project_dir = out_dir + '/' + project
# if not os.path.exists(project_dir):
#     os.makedirs(project_dir)
